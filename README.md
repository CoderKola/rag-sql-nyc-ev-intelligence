...
```mermaid
flowchart TD
    Q([Question]) --> G{EV Guard}
    G -- fail --> X([Rejected])
    G -- pass --> KB

    KB[(ChromaDB Knowledge Base\nSchema · 21 columns\nAnalyst reasoning framework\nData gotchas + edge cases\nProgram context + pricing)] --> R{Route}

    R -- analytics --> A[DeepSeek\nSQL generation]
    R -- nearest --> B[Nominatim geocode\nHaversine SQL]
    R -- elaborate --> C[Re-run prior SQL]
    R -- info / how / price --> D[DeepSeek\nRAG direct answer]

    A --> DB[(DuckDB · Parquet)]
    B --> DB
    C --> DB
    DB -- error --> FX[DeepSeek auto-fix] --> DB
    DB -- ok --> I[DeepSeek\ninterpret + visualize]
    D --> OUT

    I --> OUT([Text · Plotly Chart · MapLibre Map])
```
```
...
```mermaid
flowchart TD
    A[User Query] --> B[FastAPI · SSE stream]
    B --> C[Guard · EV topic classifier]
    C --> D[ChromaDB · RAG retrieval]
    D --> E{Route Classifier}

    E -->|program info| F[Direct · DeepSeek + RAG context]
    E -->|nearest charger| G[Nominatim · OSM geocode]
    G --> H[DuckDB · Haversine distance SQL]
    E -->|elaborate prior| I[Re-run prior SQL · DuckDB]
    E -->|analytics| J[DeepSeek · SQL generation]
    J --> K[DuckDB · Parquet scan]
    K --> L[DeepSeek · result interpretation]
    I --> L

    L --> M[Answer + optional Plotly chart]
    H --> N[Answer + MapLibre pin map]
    F --> O[Answer · RAG-grounded]

    M --> P[SvelteKit · streamed to UI]
    N --> P
    O --> P
```
